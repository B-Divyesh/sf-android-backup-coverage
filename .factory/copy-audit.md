...
| Concept | One term |
|---|---|
| Folder on the phone | phone folder |
| Compared destination | backup copy |
| Comparison process | backup check |
| Saved or displayed result | backup receipt |
| Imported JSON evidence | file list |
| Equal path and size | verified |
| Past the arrival window | missing |
| Same path, different size | changed |

`Android Backup Coverage` and the short wordmark `Backup Coverage` are proper product names.
